...
class Database:
    ...
    def add_parameter(self, parameter_data, init=False):
        ...
        value_str = (
            json.dumps(parameter_data.value)
            if isinstance(parameter_data.value, dict)
            else str(parameter_data.value)
        )
            ...
            if not init:
                self.conn.execute(
                    ...
                    (
                        ...
                        value_str,  # Use the serialized string
    ...
    def get_parameter(self):
        ...
        for row in rows:
            ...
            try:
                param_dict["value"] = json.loads(
                    param_dict["value"]
                )  # Deserialize JSON string to dict
            except json.JSONDecodeError:
                pass  # Keep value as is if not JSON
